            ...
            <table className="sm:min-w-full  bg-white border-collapse">
                <thead>
                    <tr className="border-b-2 border-gray-300">
                        <th className="sm:px-6 px-1 py-4 text-left">Date</th>
                        <th className="sm:px-6 px-1 py-4 text-left">Job Role</th>
                        <th className="sm:px-6 px-1 py-4 text-left">Company</th>
                        <th className="sm:px-6 px-1 py-4 text-left">Status</th>
                    </tr>
                </thead>
                <tbody>
                    {
                        !appliedJobsByStudent ? <span className='text-center'>You have not applied any Job yet</span> : appliedJobsByStudent.map((appliedJob) => (
                            <tr key={appliedJob._id} className="border-b border-gray-300">
                                <td className="sm:px-6 px-1 py-4 text-left">{appliedJob.createdAt.split("T")[0]}</td>
                                <td className="sm:px-6 px-1 py-4 text-left">{appliedJob?.job?.title}</td>
                                <td className="sm:px-6 px-1 py-4 text-left">{appliedJob?.job?.company?.name}</td>
                                <td className="sm:px-6 px-1 py-4 text-left">
